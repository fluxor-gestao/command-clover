...
SELECT 
    ...
    COALESCE(SUM(CASE WHEN i.due_date >= CURRENT_DATE THEN i.outstanding_amount ELSE 0 END), 0) AS future_receivable,
    MAX(i.due_date) AS last_installment_due,
    GREATEST((o.initial_capital + COALESCE((SELECT SUM(amount) FROM investment_contributions WHERE operation_id = o.id AND cancelled_at IS NULL), 0)) - COALESCE(SUM(i.received_amount), 0), 0) AS capital_to_recover,
    CASE 
        WHEN (o.initial_capital + COALESCE((SELECT SUM(amount) FROM investment_contributions WHERE operation_id = o.id AND cancelled_at IS NULL), 0)) > 0 
        THEN LEAST(COALESCE(SUM(i.received_amount), 0) / (o.initial_capital + COALESCE((SELECT SUM(amount) FROM investment_contributions WHERE operation_id = o.id AND cancelled_at IS NULL), 0)), 1.0)
        ELSE 0 
