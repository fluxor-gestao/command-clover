...
import { listAnnualSheets, normalizeReference, parseWorkbook, } from "./parse-workbook";
/** Lista as abas anuais disponíveis no arquivo, sem gravar nada. */
export async function inspectWorkbookFile(file) {
...
export async function readWorkbookFile(file, options) {
    const workbook = new ExcelJS.Workbook();
    await workbook.xlsx.load(await file.arrayBuffer());
 ...
 */
export async function importParseResult(filename, result, mode, onProgress, options) {
    ...
    for (const op of result.operations) {
        ...
        if (existing) {
            ...
            syncStatus = status || "ALTERADO_NO_EXCEL";
        }
        ...
        if (op.isManagement && operationId) {
            ...
            if (yearMatch) {
                ...
                await supabase
                    ...
                    .upsert({ operation_id: operationId, portfolio_year: year, is_active: true }, { onConflict: "operation_id,portfolio_year" });
            }
        }
        ...
        if (op.contributions.length > 0) {
            const { error: contribError } = await supabase.from("investment_contributions").upsert(op.contributions.map((c) => ({
                operation_id: operationId,
    ...
    if (result.issues.length > 0) {
        ...
        for (let i = 0; i < result.issues.length; i += chunkSize) {
            const { error } = await supabase.from("investment_import_issues").insert(result.issues.slice(i, i + chunkSize).map((issue) => ({
                import_id: importId,
