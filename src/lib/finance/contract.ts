...
export type ContractMode = "PRIMEIRO_VENCIMENTO" | "DATA_FINAL";

const lastDayOfMonth = (year: number, month: number) => new Date(Date.UTC(year, month + 1, 0)).getUTCDate();

export function addMonthsClamped(iso: string, months: number, dueDay?: number | null): string {
...
export function deriveContractDates(input: ContractDatesInput): ContractDates {
...
export interface SimulationResult {
  ...
  profit: number;
  roiTotal: number;
  roiMonthlyAverage: number;
  paybackInstallments: number | null;
  paybackDate: string | null;
  finalDate: string | null;
  schedule: SimulationRow[];
}

export function simulateContract(input: SimulationInput): SimulationResult {
  ...
  const schedule: SimulationRow[] = [];
  let accumulated = 0;
  let paybackInstallments: number | null = null;

  for (let i = 1; i <= count; i += 1) {
    accumulated += value;
    const dueDate = input.firstDueDate ? addMonthsClamped(input.firstDueDate, i - 1, input.dueDay) : "";
    if (paybackInstallments === null && capital > 0 && accumulated >= capital) paybackInstallments = i;
    schedule.push({
      number: i,
      dueDate,
      amount: value,
