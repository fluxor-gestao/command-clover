  ...
  const projectedRows = useMemo(() => {
    ...
    return months.map(month => {
      ...
      operations.data.forEach(op => {
        if (op.is_own_property || !op.first_due_date || !op.installment_count || !op.installment_value) return;
        
        const simulation = simulateContract({
          capital: op.initial_capital ?? 0,
          installmentValue: op.installment_value,
          installmentCount: op.installment_count,
          firstDueDate: op.first_due_date,
          dueDay: op.due_day
        });
