...
import { useInstallments, useMonthlyFlow, useOperations, usePortfolioMetrics, usePortfolioProjection, useReceivedInMonth, } from "@/lib/data/hooks";
...
export const Route = createFileRoute("/_authenticated/dashboard")({
    head: () => ({
        meta: [
            ...
            {
                name: "description",
                content: "Painel executivo com capital investido, recebido, saldo a recuperar, inadimplência e retorno da carteira Nova Era.",
            },
...
function DashboardPage() {
    ...
    const scopedInstallments = (installments.data ?? []).filter((row) => year === null ? true : String(row.competence ?? "").slice(0, 4) === String(year));
    const scopedOperationIds = new Set(scopedInstallments.map((row) => row.operation_id));
    const overdueByOperation = scopedInstallments.reduce((acc, row) => {
        const outstanding = Number(row.outstanding_amount ?? 0);
        if (outstanding <= 0 || !row.operation_id)
            return acc;
    ...
    const scopedOperations = (operations.data ?? []).filter((op) => year === null || (op.operation_id != null && scopedOperationIds.has(op.operation_id)));
    const byCategory = Object.values(scopedOperations.reduce((acc, op) => {
    ...
    return (<div className="space-y-6">
      <header className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            {year === null
            ? "Audit Mode — Visão completa e histórica de toda a base de dados."
            : `Management Mode — Indicadores oficiais da Carteira ${year}.`}
          </p>
        </div>
      ...
      <section className="grid gap-6 md:grid-cols-2 lg:grid-cols-4">
        <Kpi title="Capital investido" value={brl(s?.capital_investido)} hint={`${s?.total_operations ?? 0} ativos`} icon={<Wallet className="size-4"/>} primary onClick={() => navigate({ to: "/operacoes" })}/>
        <Kpi title="Capital recebido" value={brl(s?.total_received)} hint={`${pct((s?.percentual_recuperado ?? 0) / 100)} do principal`} icon={<ArrowDownRight className="size-4"/>} primary onClick={() => navigate({ to: "/recebimentos" })}/>
        <Kpi title="Capital a Recuperar" value={brl(s?.capital_a_recuperar)} hint="Saldo pendente do aporte" icon={<ArrowUpRight className="size-4"/>} primary onClick={() => navigate({ to: "/operacoes", search: { status: "INADIMPLENTE" } })}/>
        <Kpi title="Total a Receber" value={brl(s?.total_a_receber)} hint="Saldo aberto (Inad. + Futuro)" icon={<TrendingUp className="size-4"/>} primary onClick={() => navigate({ to: "/parcelas" })}/>
      </section>

      <section className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        <Kpi title="Lucro realizado" value={brl(s?.lucro_realizado)} hint="Excedente ao principal" onClick={() => navigate({ to: "/relatorios" })}/>
        <Kpi title="Resultado Projetado" value={brl(s?.resultado_projetado)} hint="Previsto - Investido" onClick={() => navigate({ to: "/relatorios" })}/>
        <Kpi title="Inadimplência" value={brl(s?.inadimplencia)} hint="Parcelas vencidas em aberto" tone="destructive" onClick={() => navigate({ to: "/parcelas", search: { status: "VENCIDA" } })}/>
        <Kpi title="A Receber Futuro" value={brl(s?.a_receber_futuro)} hint={`Recebido no mês: ${brl(receivedInMonth.data ?? 0)}`} onClick={() => navigate({ to: "/relatorios" })}/>
      </section>
      ...
      <Card className="border-none shadow-sm bg-card/50">
        ...
        <CardContent className="pt-4">
          <Progress value={Number(s?.percentual_recuperado ?? 0)} className="h-3 bg-muted"/>
          ...
          <CardContent className="h-80">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={monthly}>
                ...
                <YAxis fontSize={11} tickFormatter={(v) => brlCompact(v)}/>
            ...
            <ResponsiveContainer width="100%" height="100%">
              <PieChart>
                <Pie data={byCategory} dataKey="value" nameKey="name" innerRadius={55} outerRadius={95}>
                  {byCategory.map((entry, index) => (<Cell key={entry.name} fill={CHART_COLORS[index % CHART_COLORS.length]}/>))}
                </Pie>
                <Tooltip formatter={(value) => brl(value)}/>
                <Legend />
              </PieChart>
          ...
          <CardContent className="h-72">
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={monthly}>
                ...
                <YAxis fontSize={11} tickFormatter={(v) => brlCompact(v)}/>
                <Tooltip formatter={(value) => brl(value)}/>
                <Line type="monotone" dataKey="inadimplente" name="Inadimplente" stroke="oklch(0.605 0.22 28.27)" strokeWidth={2} dot={false}/>
              </LineChart>
            </ResponsiveContainer>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            <Table>
              ...
              <TableBody>
                {topOverdue.length === 0 && (<TableRow>
                    <TableCell colSpan={3} className="text-sm text-muted-foreground">
                      Nenhuma parcela vencida em aberto.
                    </TableCell>
                  </TableRow>)}
                {topOverdue.map((op) => (<TableRow key={op.operation_id} className="group cursor-pointer transition-colors hover:bg-muted/50" onClick={() => navigate({ to: "/operacoes/$id", params: { id: op.operation_id ?? "" } })}>
...
function Kpi({ title, value, hint, icon, tone, primary, onClick, }) {
    return (<Card onClick={onClick} className={cn("border-none shadow-sm transition-all hover:shadow-md", onClick && "cursor-pointer active:scale-[0.98]", primary ? "bg-primary text-primary-foreground" : "bg-card", tone === "destructive" && !primary && "bg-destructive/5")}>
      <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
        <CardTitle className={cn("text-xs font-bold uppercase tracking-wider", primary ? "text-primary-foreground/70" : "text-muted-foreground")}>
          {title}
        </CardTitle>
        {icon && (<div className={cn("rounded-md p-1.5", primary ? "bg-primary-foreground/10" : "bg-muted")}>
            {icon}
          </div>)}
      </CardHeader>
      <CardContent>
        <div className="flex flex-col gap-1">
          <p className={cn("text-2xl font-bold tracking-tight", tone === "destructive" && !primary ? "text-destructive" : "")}>
            {value}
          </p>
          {hint && (<p className={cn("text-[10px] font-medium uppercase tracking-tight opacity-70", primary ? "text-primary-foreground/70" : "text-muted-foreground")}>
              {hint}
            </p>)}
        </div>
      </CardContent>
