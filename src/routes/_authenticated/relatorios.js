...
function ReportsPage() {
    ...
    const projectedRows = useMemo(() => {
        ...
        if (!year || !operations.data)
            return realRows.filter(r => !year || r.competence?.startsWith(String(year)));
        ...
        return months.map(month => {
            ...
            if (existing && (existing.installments_count ?? 0) > 0)
                return existing;
            ...
            operations.data?.forEach(op => {
                const rawOp = op;
                if (rawOp.is_own_property || !rawOp.first_due_date || !rawOp.installment_count || !rawOp.installment_value)
                    return;
    ...
    return (<div className="space-y-8">
      ...
      <section className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        ...
        ].map((item) => (<Card key={item.label} className="border-none shadow-sm bg-card/50">
          ...
          <Card className="border-none shadow-sm overflow-hidden bg-card/50">
            ...
            <CardContent className="p-0 overflow-x-auto">
              <Table>
                ...
                <TableBody>
                  {rows.map((row) => (<TableRow key={row.competence} className="group hover:bg-muted/50 transition-colors border-border/40">
                      <TableCell className="font-bold py-4 pl-6 flex items-center gap-2">
                        {competenceBR(row.competence)}
                        {row.isProjected && (<Badge variant="secondary" className="h-4 text-[8px] px-1 bg-primary/5 text-primary border-primary/20">
                      ...
                      <TableCell className="text-right tabular-nums">
                        <Badge variant="outline" className={cn("h-5 font-bold border-none", (row.realization_percentage ?? 0) >= 90 ? "bg-success/10 text-success" :
